...
class Particle(object):
    ...
    def get_color(self):
        if self._padlib_color_needs_update:
            ...
            part = part - int(part)
            self.color = [rndint(color1[i] + part * delta[i]) for i in
                          [0, 1, 2]]
    ...
    def draw(self, surface):
        surface.set_at((rndint(self.position[0]), rndint(self.position[1])),
                       self.get_color())
    ...
    def _padlib_update(self, parent, dt):
        for i in range(self.density):
            if random.random() < dt:
                ...
                parent.particles.append(
                    Particle(parent, pos, vel, life, self.colors))
