...
def draw(surface, color, rect, radius, width):
    ...
    pygame.draw.rect(surf_temp, color,
                     (0, radius, rect[2], rect[3] - 2 * radius), 0)
    pygame.draw.rect(surf_temp, color,
                     (radius, 0, rect[2] - 2 * radius, rect[3]), 0)
    ...
    if width != 0:
        draw(surf_temp, colorkey,
             (width, width, rect[2] - 2 * width, rect[3] - 2 * width),
             radius - width, 0)
