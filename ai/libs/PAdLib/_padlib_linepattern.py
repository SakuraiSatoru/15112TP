...
def draw(surface, p1, p2, shading_function, section_length, section_offset):
    ...
    while True:
        ...
        surface.set_at((x0, y0), shading_function(
            (displacement % section_length) / section_length))
...
def aadraw(surface, p1, p2, shading_function, section_length, section_offset,
           blend):
    ...
    def plot(x, y, c):
        ...
        color2 = shading_function(
            (displacement % section_length) / float(section_length))
        if blend:
            color1 = surface.get_at((x, y))
            color = [rndint(color1[i] * (1.0 - c) + c * color2[i]) for i in
                     [0, 1, 2]]
        else:
