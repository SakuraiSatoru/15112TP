...
def trianglecustom(surface, p1, p2, p3, shading_function, is_numpy):
    ...
    size = (int(ceil(maxs[0] - mins[0])), int(ceil(maxs[1] - mins[1])))
    ...
    if is_numpy:
        ...
        arr_temp[:, :, 1] = transformation_matrix[0] * coords_x + \
                            transformation_matrix[1] * coords_y  # set b
        arr_temp[:, :, 2] = transformation_matrix[2] * coords_x + \
                            transformation_matrix[3] * coords_y  # set c
        arr_temp[:, :, 3] = arr_temp[:, :, 1] + arr_temp[:, :, 2]  # set b+c
        arr_temp[:, :, 0] = 1.0 - arr_temp[:, :,
                                  3]  # set a (which is 1.0-b-c which is 1.0-(b+c))

        # Put boolean whether the barycentric coordinates are inside
        np.logical_and(arr_temp[:, :, 1] >= 0.0, arr_temp[:, :, 2] >= 0.0,
                       arr_in[:, :])
        np.logical_and(arr_in[:, :], arr_temp[:, :, 3] <= 1.0, arr_in[:, :])

        # Shade
        shading_function(arr_temp[:, :, 0], arr_temp[:, :, 1],
                         arr_temp[:, :, 2], arr_in, arr_surf_bgra)
        ...
        for y in range(size[1]):
            ...
            for x in range(size[0]):
                ...
                b = transformation_matrix[0] * p[0] + transformation_matrix[
                    1] * p[1]
                if b < 0.0: continue
                c = transformation_matrix[2] * p[0] + transformation_matrix[
                    3] * p[1]
                if c < 0.0: continue
                if b + c > 1.0: continue
                a = 1.0 - b - c

                temp_surf.set_at((x, y),
                                 list(map(rndint, shading_function(a, b, c))))
...
def triangletexture(surface, texture, t1, t2, t3, p1, p2, p3, filter,
                    should_clamp):
    ...
    def sample_nc(sx, sy):  # nearest, with clamp
        return texture.get_at(
            (clamp(rndint(sx * w), 0, w1), clamp(rndint(sy * h), 0, h1)))
    ...
    def sample_bc(sx, sy):  # bilinear, with clamp
        sx *= w;
        sy *= h
        # can't use int(...) all by itself; rounds toward 0, not -inf.
        isx0 = int(floor(sx));
        isy0 = int(floor(sy));
        isx1 = isx0 + 1;
        isy1 = isy0 + 1
        ...
        px1 = 1.0 - px;
        py1 = 1.0 - py
        isx0 = clamp(isx0, 0, w1);
        isx1 = clamp(isx1, 0, w1);
        isy0 = clamp(isy0, 0, h1);
        isy1 = clamp(isy1, 0, h1)
        c00 = texture.get_at((isx0, isy0))
        c10 = texture.get_at((isx1, isy0))
        c01 = texture.get_at((isx0, isy1))
        c11 = texture.get_at((isx1, isy1))
        return [rndint((c00[i] * px1 + c10[i] * px) * py1 + (
                c01[i] * px1 + c11[i] * px) * py) for i in [0, 1, 2, 3]]
    ...
    if filter:
        if should_clamp:
            sample_func = sample_bc
        else:
            sample_func = sample_br
    else:
        if should_clamp:
            sample_func = sample_nc
        else:
            sample_func = sample_nr
...
def quadtexture(surface, texture, t1, t2, t3, t4, p1, p2, p3, p4, filter,
                clamp):
