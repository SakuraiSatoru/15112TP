        ...
        if self._padlib_needs_update:
            ...
            if not inside:
                pygame.draw.circle(self.mask, (255, 255, 255),
                                   [rndint(self.radius)] * 2,
                                   rndint(self.radius), 0)
                ...
                for occluder in self.occluders:
                    for i in range(occluder.numof_points):
                        ...
                        if vec_dot(delta1, occluder._padlib_normals[
                            i]) > 0.0:  # backface
                            l1 = occluder.points[
                                (i + 1) % occluder.numof_points]
                            ...
                            def get_side(point):  # arranged counterclockwise
                                ...
                                if abs(point[
                                           0] - self.mask.get_width()) < 1.0: return 2  # right
                            ...
                            while True:
                                ...
                                if s3 == s4:
                                    ...
                                    if s3 == 0:
                                        p3 = [0.0, self.mask.get_height()]
                                    elif s3 == 1:
                                        p3 = self.mask.get_size()
                                    elif s3 == 2:
                                        p3 = [self.mask.get_width(), 0.0]
                                    else:
                                        p3 = [0.0, 0.0]
                                    s3 = (s3 + 1) % 4
                            poly.append(p4)
                            ...
                            pygame.draw.polygon(
                                ...
                                (0, 0, 0),
                                list(
                                    map(lambda p: (rndint(p[0]), rndint(p[1])),
                                        poly)),
                                0
                            )
                    if fill_occluders:
                        pygame.draw.polygon(
                            self.mask,
                            (0, 0, 0),
                            [[rndint(point[i] - self.position[i] + self.radius)
                              for i in [0, 1]] for point in occluder.points],
                            0
                        )
            self._padlib_needs_update = False
        return self.mask, (rndint(self.position[0] - self.radius),
                           rndint(self.position[1] - self.radius))
